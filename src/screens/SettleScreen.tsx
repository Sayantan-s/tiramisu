...
export function SettleScreen() {
  ...
  const months = useMemo(() => {
    ...
    for (const e of expenses) set.add(monthKey(e.paid_at));
    for (const s of settlements) set.add(monthKey(s.paid_at));
  ...
  const monthExpenses = useMemo(
    () => expensesInMonth(expenses, activeMonth),
    [expenses, activeMonth],
  );
  const monthSettlements = useMemo(
    () => settlementsInMonth(settlements, activeMonth),
    [settlements, activeMonth],
  );
  const balances = useMemo(
    () => computeBalancesFromDto(monthExpenses, monthSettlements, memberIds),
    [monthExpenses, monthSettlements, memberIds],
  );
  ...
  const handleUpi = (toId: string, amount: number) => {
    const url = `upi://pay?pa=demo@upi&pn=${encodeURIComponent(memberName(toId))}&am=${paiseToRupees(
      ...
      `Settle for ${formatMonth(`${activeMonth}-01T00:00:00.000Z`)}`,
    )}`;
  ...
  const shareSummary = async () => {
    if (!group) return;
    const lines: string[] = [];
    lines.push(`🧾 ${group.name} — ${formatMonth(`${activeMonth}-01T00:00:00.000Z`)}`);
