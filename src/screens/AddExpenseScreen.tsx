...
import { validateSplit, type SplitRule } from '../lib/split';
import type { GroupStackParamList } from '../navigation/types';

type Mode = 'equal' | 'shares' | 'percent' | 'exact' | 'itemized';
type Category = 'rent' | 'utilities' | 'groceries' | 'household' | 'food' | 'other';
...
const CATEGORIES: ReadonlyArray<{ value: Category; label: string; emoji: string }> = [
...
type R = RouteProp<GroupStackParamList, 'AddExpense'>;
...
export function AddExpenseScreen() {
  ...
  const [payerId, setPayerId] = useState<string>(
    prefill?.payer_id ?? me?.id ?? members[0]?.id ?? '',
  );
  const [amountStr, setAmountStr] = useState<string>(
    prefill?.amount != null ? (prefill.amount / 100).toString() : '',
  );
  const [description, setDescription] = useState(prefill?.description ?? '');
  const [category, setCategory] = useState<Category>((prefill?.category as Category) ?? 'groceries');
  ...
  const canSave = validation.ok && amountPaise > 0 && payerId.length > 0 && !busy;
  ...
  const save = async () => {
    ...
    try {
      await addExpense({
        ...
        receipt_uri: prefill?.receipt_uri ?? null,
  ...
  return (
    <Screen scrollable padded>
      <Stack gap={5}>
            ...
            {CATEGORIES.map((c) => {
              ...
              return (
                <Pressable
                  ...
                  <Text variant="caption" weight="600" tone={active ? 'inverse' : 'default'}>
                    {c.emoji} {c.label}
                  </Text>
                </Pressable>
              );
            ...
            {members.map((m) => {
              ...
              return (
                <Pressable
                  ...
                  style={{ alignItems: 'center', gap: theme.spacing(1), padding: theme.spacing(1) }}>
        ...
        {(!validation.ok && amountPaise > 0) || error ? (
        ...
        <Stack gap={2}>
          ...
          <Button title={busy ? 'Saving…' : 'Save expense'} fullWidth disabled={!canSave} onPress={save} />
        </Stack>
      </Stack>
    </Screen>
  );
